...
export BAMDIR OUTBASE LIST_P53_loss LIST_P53_SETD2i REF LIST_RPE_control LIST_RPE_SETD2i
...
parallel --tmpdir "$TMPDIR" --jobs 5 --halt soon,fail=10 '
    ...
    BAM="$BAMDIR/RPE_control/${SAMPLE}.marked.bam"

    echo "Variant calling with bcftools for $SAMPLE..."

    bcftools mpileup -O v -f "$REF" "$BAM" | bcftools call -vm -O v > "$OUTBASE/RPE_control/${SAMPLE}.variants.vcf"
...
parallel --dry-run --tmpdir "$TMPDIR" --jobs 5 --halt soon,fail=10 '
    ...
    BAM="$BAMDIR/RPE_SETD2i/${SAMPLE}.marked.bam"

    echo "Variant calling with bcftools for $SAMPLE..."

    bcftools mpileup -O v -f "$REF" "$BAM" | bcftools call -vm -O v > "$OUTBASE/RPE_SETD2i/${SAMPLE}.variants.vcf"
