...
3.1 - Preprocess and annotate Target Intervals 
-   Obs: GATK requires a FASTA dictionary file (.dict) alongside your reference FASTA (.fa)
-   Solve: Run `gatk CreateSequenceDictionary` in ref folder. Output in the same location
````
...
3.3 - Create Panel of Normals (PoN)
-   script: `GATK_panel_of_normals.sh`

3.4 - Denoise Read Counts
...
-    Table:
| MEAN_LOG2_COPY_RATIO | Interpretation                 |
| -------------------- | ------------------------------ |
| ~0                   | Normal diploid (2 copies)      |
| +0.58                | ~3 copies                      |
